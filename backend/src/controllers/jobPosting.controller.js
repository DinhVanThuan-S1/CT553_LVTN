...
const { createAndEmitNotification, notifyAdmins } = require('./notification.controller');
...
exports.createJob = async (req, res) => {
  try {
    ...
    // Notify all admins nếu tin được gửi duyệt (không phải nháp)
    if (data.status === 'pending') {
      try {
        ...
        await notifyAdmins({
          ...
          content: `"${data.title}" — được đăng bởi ${employer.fullName || employer.email}. Vui lòng kiểm tra và duyệt.`,
          link: '/admin/job-postings',
          refModel: 'JobPosting',
          refId: data._id,
        });
      } catch (notifErr) {
        console.error('Notification error (createJob → admins):', notifErr);
      }
    }
  } catch (error) {
    res.status(error.status || 500).json({ success: false, message: error.message });
...
exports.updateJob = async (req, res) => {
  try {
    ...
    // Notify admins nếu NTD re-submit tin bị từ chối
    if (data.status === 'pending') {
      try {
        const employer = req.user;
        await notifyAdmins({
          type: 'new_job_pending',
          title: 'Tin tuyển dụng chờ duyệt lại',
          content: `"${data.title}" — được cập nhật và gửi duyệt lại bởi ${employer.fullName || employer.email}.`,
          link: '/admin/job-postings',
          refModel: 'JobPosting',
          refId: data._id,
        });
      } catch (notifErr) {
        console.error('Notification error (updateJob → admins):', notifErr);
      }
    }
  } catch (error) {
    res.status(error.status || 500).json({ success: false, message: error.message });
